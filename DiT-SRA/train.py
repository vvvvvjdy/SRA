...
def main(args):
    ...
    # For DiTs, we do not sample the images during training.
    for epoch in range(epoch_start+1, args.epochs):
        ...
        for images_l, y in train_dataloader:
            ...
            with accelerator.accumulate(model):
                ...
                t_max = int(diffusion.num_timesteps * args.t_max)
                loss_dict = diffusion.training_losses(model,x,t,labels,ema,args.block_out_s,args.block_out_t,t_max)
                gen_loss_mean = loss_dict["loss"].mean()
                ...
                if args.resolution == 256:
                    align_loss_mean = align_loss.mean() * 0.4 * (0.1 ** (((epoch - 149) / 650 + 1) if epoch > 149 else 1))
                else:
                    align_loss_mean = align_loss.mean() * 0.4 * (0.1 ** (((epoch - 99) / 300 + 1) if epoch > 99 else 1))
...
def parse_args(input_args=None):
    ...
    # loss
    parser.add_argument("--loss-type", type=str, default="sml1", choices=["sml1"], help="for DiT we only use sml1 loss")
    parser.add_argument("--cfg-prob", type=float, default=0.1, help="use class-free guidance if > 0.0")
